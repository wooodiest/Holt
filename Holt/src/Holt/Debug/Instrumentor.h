	...
	public:
		...
		void BeginSession(const std::string& name, const std::string& filepath = "results.json")
		{
			...
			if (m_CurrentSession)
			{
				...
				{
					HL_CORE_ERROR("Instrumentor::BeginSession('{0}') when session '{1}' already open.", name, m_CurrentSession->Name);
				}
				InternalEndSession();
			}
...
// could mark the wrong one in your editor!
	...
	#define HL_PROFILE_BEGIN_SESSION(name, filepath) ::Holt::Instrumentor::Get().BeginSession(name, filepath)
	#define HL_PROFILE_END_SESSION() ::Holt::Instrumentor::Get().EndSession()
	#define HL_PROFILE_SCOPE_LINE2(name, line) constexpr auto fixedName##line = ::Holt::InstrumentorUtils::CleanupOutputString(name, "__cdecl ");\
												   ::Holt::InstrumentationTimer timer##line(fixedName##line.Data)
