...
#include "Holt/Math/Math.h"
#include "Holt/Scene/SceneSerializer.h"
#include "Holt/Utils/PlatformUtils.h"
...
namespace Holt {
	...
	void EditorLayer::OnAttach()
	{
		...
		serializer.Deserialize("assets/scenes/PinkCube.holt");
	...
	{
		std::string filepath = FileDialogs::OpenFile("Holt Scene (*.holt)\0*.holt\0");
		if (!filepath.empty())
		{
			m_ActiveScene = CreateRef<Scene>();
			m_ActiveScene->OnViewportResize((uint32_t)m_ViewportSize.x, (uint32_t)m_ViewportSize.y);
	...
	{
		std::string filepath = FileDialogs::SaveFile("Holt Scene (*.holt)\0*.holt\0");
		if (!filepath.empty())
		{
			SceneSerializer serializer(m_ActiveScene);
